...
def setup_page():
    ...
    # Add local logo handling via JS
    st.markdown("""
    <script>
    ...
    <div class="main-header">
        <div class="header-container">
            ...
            <div class="title-container" style="text-align: center;">
                <h1>HireNexus - Intelligent Hiring Agent</h1>
...
def setup_sidebar():
    with st.sidebar:
        ...
        <div style="text-align: center; margin-top: 20px;">
            <p> HireNexus – Intelligent Hiring Agent</p>
...
def role_selection_section(role_requirements):
    ...
    custom_jd = None
    if upload_jd:
        custom_jd_file = st.file_uploader("Upload job description (PDF or TXT)",
            type=["pdf", "txt"])
        if custom_jd_file:
    ...
    if not upload_jd:
        ...
        st.markdown(f"<p>Cutoff Score for selection: <b>{75}/100</b></p>",
            unsafe_allow_html=True)
...
def display_analysis_results(analysis_result):
    ...
    if missing_skills:
        for skill in missing_skills:
            st.markdown(
                f'<div class="skill-tag missing">{skill} ({skill_scores.get(skill, "N/A")}/10)</div>',unsafe_allow_html=True)
    else:
    ...
    if detailed_weaknesses:
        ...
        for weakness in detailed_weaknesses:
            skill_name = weakness.get('skill_name', '')
...
"""
        ...
        report_content += "\n---\nAnalysis provided by HireNexus Intelligent Hiring Agent"
        
        report_b64 = base64.b64encode(report_content.encode()).decode()
        href = f'<a class="download-btn" href="data:text/plain;base64,{report_b64}" download="hirenexus_resume_analysis.txt">📄 Download Analysis Report</a>'
...
def interview_questions_section(has_resume, generate_questions_func=None):
    ...
    if st.button("Generate Interview Questions"):
        if generate_questions_func:
            ...
            # Add HireNexus branding to download content
            download_content += "\n\nQuestions generated by HireNexus Intelligent Hiring Agent"
    ...
    if st.button("Generate Resume Improvements"):
        if improve_resume_func:
            with st.spinner("Analyzing and generating improvements..."):
                ...
                # Add HireNexus branding to download content
                download_content += "\n---\nProvided by HireNexus Intelligent Hiring Agent"
...
---
Resume enhanced by HireNexus Intelligent Hiring Agent
"""
                    md_bytes = md_content.encode()
                    md_b64 = base64.b64encode(md_bytes).decode()
                    md_href = f'<a class="download-btn" href="data:text/markdown;base64,{md_b64}" download="hirenexus_improved_resume.md">📄 Download as Markdown</a>'
